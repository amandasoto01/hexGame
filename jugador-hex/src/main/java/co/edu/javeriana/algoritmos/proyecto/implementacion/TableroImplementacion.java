...
public class TableroImplementacion implements Tablero{
	...
	public ColorJugador ganador() {
		/**
		 * Realizar BFS desde la columna 0 (para todas las que sean blancas)
		 * y ver si en algun momento se llega a la columna 11
		 * en ese caso el ganador es el color BLANCO
		 * 
		 * Realizar BFS desde la fila 0 (para todas las que sean negras)
		 * y ver si en algun momento se llega a la fila 11
		 * en ese caso el ganador es el color NEGRO
		 **/
		...
		while(!cola.isEmpty()) {
			...
			if(tope.getSecond() == 11) {
				return ColorJugador.BLANCO;
			}
			
			for(int i = 0; i<utilsImplementacion.numMovimientos; i++) {
				if(utilsImplementacion.posicionValida(
						tope.getFirst() + utilsImplementacion.movimientosX[i],
						tope.getSecond() + utilsImplementacion.movimientosY[i])
						&& tableroHex[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope.getSecond() + utilsImplementacion.movimientosY[i]] != null) {
					if(tableroHex[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope.getSecond() + utilsImplementacion.movimientosY[i]] == ColorJugador.BLANCO
							&& visitados[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope.getSecond() + utilsImplementacion.movimientosY[i]] == false) {
						cola.add(new Pair(tope.getFirst() + utilsImplementacion.movimientosX[i], tope.getSecond() + utilsImplementacion.movimientosY[i]));
					}
				}
			}
		}
