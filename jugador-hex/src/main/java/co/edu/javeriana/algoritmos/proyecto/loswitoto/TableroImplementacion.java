...
public class TableroImplementacion implements Tablero {
    ...
    public void aplicarJugada(Jugada jugada, ColorJugador colorJugador) {
        if (jugada.isCambioColores()) {
            /**
             * Solo se hace un cambio de colores de los jugadores pero el tablero se mantiene igual
             **/
        } else {
            /**
    ...
    public ColorJugador ganador() {
        /**
         * Realizar BFS desde la columna 0 (para todas las que sean blancas) y ver si en algun
         * momento se llega a la columna 10 en ese caso el ganador es el color BLANCO
         * 
         * Realizar BFS desde la fila 0 (para todas las que sean negras) y ver si en algun momento
         * se llega a la fila 10 en ese caso el ganador es el color NEGRO
         **/
        ...
        while (!cola.isEmpty()) {
            ...
            for (int i = 0; i < utilsImplementacion.numMovimientos; i++) {
                if (utilsImplementacion.posicionValida(
                        ...
                        && tableroHex[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope
                                .getSecond() + utilsImplementacion.movimientosY[i]] != null) {
                    if (tableroHex[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope
                            .getSecond()
                            + utilsImplementacion.movimientosY[i]] == ColorJugador.BLANCO
                            && visitados[tope.getFirst() + utilsImplementacion.movimientosX[i]][tope
                                    .getSecond() + utilsImplementacion.movimientosY[i]] == false) {
